...
import { z } from "zod";
import { toast } from "react-toastify";
import { CheckCircle, Clock, Download, Filter, Mail, Phone, Search, Shield, UserPlus, Users, XCircle, Key, Lock, Unlock } from "lucide-react";
import { getAllAdmins, createAdmin, resendAdminCode, deleteAdmin, toggleAdminStatus } from "../../services/api.admin.js";
import { LiquidButton } from "@/components/ui/liquid.js";
...
export default function SuperAdminDashboard() {
  ...
  const getStatusBadge = (admin) => {
    ...
    // First check if admin is manually disabled
    ...
    // Check if admin has logged in (most reliable indicator of active)
    ...
    // Default to active if verified (even if haven't logged in yet)
    return (
      <div className="flex items-center gap-1">
        <CheckCircle className="h-3 w-3" />
        <span className="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs">Active</span>
  ...
  const filteredAdmins = admins.filter(admin => {
    ...
    // Get the admin status for filtering
    const isDisabled = admin.isActive === false;
    const isPending = !admin.lastLoginAt && !admin.emailVerified && admin.isActive !== false;
    const isActive = (admin.lastLoginAt || admin.emailVerified) && admin.isActive !== false;
  ...
  return (
    <div className="p-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold mt-1">
                  {admins.filter(a => !a.lastLoginAt && !a.emailVerified && a.isActive !== false).length}
                </p>
              </div>
              <Clock className="h-8 w-8 text-yellow-500" />
            </div>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center">
                          ...
                          <div className="ml-4">
                            ...
                            <div className="text-sm text-gray-500">
                              ID: {typeof admin.id === 'string' ? admin.id.substring(0, 8) + '...' : admin.id}
                            </div>
                          </div>
                        </div>
                      </td>
      ...
      {/* Create Admin Modal - Keep your existing modal code */}
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              {/* ... keep your existing modal form code ... */}
            </div>
          </div>
        </div>
      )}

      {/* Admin Details Modal - Updated with better status display */}
      {selectedAdmin && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl w-full max-w-lg">
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">ID: {typeof selectedAdmin.id === 'string' ? selectedAdmin.id.substring(0, 12) + '...' : selectedAdmin.id}</p>
                  </div>
                </div>
                ...
                <div className="flex justify-end gap-3 pt-4">
                  ...
                  {selectedAdmin.isActive !== false && !selectedAdmin.emailVerified && (
                    <button
                      onClick={() => {
                        handleResendCode(selectedAdmin.id, selectedAdmin.email);
                        setSelectedAdmin(null);
