  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const todayCheckIns = bookings.filter(b => 
        b.checkInDate === today && (b.status === "CONFIRMED" || b.status === "CHECKED_IN")
      ).length;
      
      const todayCheckOuts = bookings.filter(b => 
        b.checkOutDate === today && (b.status === "CONFIRMED" || b.status === "CHECKED_IN")
      ).length;
      ...
      const totalRoomQuantity = rooms.reduce((sum, room) => sum + (room.roomQuantity || 0), 0);
      const occupiedRooms = confirmedBookings.filter(b => 
