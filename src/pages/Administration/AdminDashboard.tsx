...
//     const userRole = admin?.role; // Get role from admin object
    
//     // console.log("=== ADMIN DASHBOARD AUTH CHECK (sessionStorage) ===");
//     // console.log("Admin from sessionStorage:", admin);
...
//     // Handle paginated response (Spring Boot returns {content: [], totalElements: x})
...
//     // Get confirmed bookings (INCLUDE WALK-INS)
...
//       return checkInDate === today && (b.status === "CONFIRMED" || b.status === "CHECKED_IN");
//     }).length;

//     const todayCheckOuts = bookings.filter(b => {
//       const checkOutDate = new Date(b.checkOutDate).toISOString().split('T')[0];
//       return checkOutDate === today && (b.status === "CONFIRMED" || b.status === "CHECKED_IN");
//     }).length;
...
//     // Calculate room availability - FIXED VERSION
//     // Sum up all room quantities from all room types
//     let totalRoomQuantity = 0;
//     rooms.forEach(room => {
//       totalRoomQuantity += room.roomQuantity || 1;
//     });
...
//         // Add room quantity (default to 1 if not found)
//         occupiedRooms += room?.roomQuantity || 1;
//       }
//     });

//     const availableRooms = Math.max(0, totalRoomQuantity - occupiedRooms);
...
//     setStats({
//       totalRooms: rooms.length, // This shows number of room types
//       availableRooms,
//       occupiedRooms,
//       totalBookings: bookings.length,
//       todayCheckIns,
...
//       // Clear sessionStorage for admin
...
//               <p className="text-gray-600">Real-time hotel management</p>
//             </div>
...
export default function AdminDashboard() {
  ...
  try {
    ...
    // ✅ FIX: Only count CONFIRMED and CHECKED_IN bookings for revenue
    // These are the bookings that have been paid or are actively using the room
    ...
    console.log("📊 Paid bookings breakdown:");
    console.log("- CONFIRMED:", bookings.filter(b => b.status === "CONFIRMED").length);
    console.log("- CHECKED_IN:", bookings.filter(b => b.status === "CHECKED_IN").length);
    console.log("- CHECKED_OUT:", bookings.filter(b => b.status === "CHECKED_OUT").length);
    console.log("- COMPLETED:", bookings.filter(b => b.status === "COMPLETED").length);
    console.log("- Total paid bookings:", paidBookings.length);
    
    // Excluded from revenue:
    console.log("📊 Excluded from revenue:");
    console.log("- PENDING_PAYMENT:", bookings.filter(b => b.status === "PENDING_PAYMENT").length);
    console.log("- CANCELLED:", bookings.filter(b => b.status === "CANCELLED").length);
    console.log("- NO_SHOW:", bookings.filter(b => b.status === "NO_SHOW").length);
    ...
    // ✅ FIXED: Calculate room availability correctly
    // 1. Calculate total rooms (sum of roomQuantity)
...
//       // ✅ FIX: Include walk-in bookings in all calculations
//       // Walk-in bookings have: isWalkIn=true, bookedByAdmin set, NO userId
      
//       // Get ALL active bookings (including walk-ins)
...
//       console.log("📊 Active bookings (including walk-ins):", activeBookings.length);
//       console.log("Walk-in bookings:", activeBookings.filter(b => b.isWalkIn || !b.userId).length);
//       console.log("User bookings:", activeBookings.filter(b => b.userId).length);
...
//       // ✅ FIX: Calculate room availability - FIXED VERSION
//       // let totalRoomQuantity = 0;
//       // rooms.forEach(room => {
//       //   totalRoomQuantity += room.roomQuantity || 1;
//       // });
...
// rooms.forEach(room => {
//   // Use availableRooms, not roomQuantity
//   totalAvailableRooms += room.availableRooms || 0;
// });
...
//       // Count occupied rooms for TODAY (INCLUDE WALK-INS)
//       let occupiedRooms = 0;
