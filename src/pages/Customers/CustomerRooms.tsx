...
export default function CustomerRooms() {
  ...
  const fetchRooms = async () => {
    ...
    try {
      ...
      // ENHANCED: Add availability calculation
      const enhancedRooms = (data || []).map(room => {
        ...
        return {
          ...
          _availabilityStatus: isAvailable ? 'available' : 'sold_out'
        };
      });
  ...
  // FIXED: Handle booking - Check for "customer" session (not "user")
  const handleBooking = (room) => {
    console.log("Attempting to book room:", room.id, room.roomName);
    
    // FIX: Check for "customer" session (consistent with CustomerBookings.jsx)
    ...
    if (customerSession) {
      try {
        ...
        customerId = customerData.id || customerData.userId || customerData.customerId;
    ...
    // Also check for direct customerId in sessionStorage
    if (!customerId) {
      customerId = sessionStorage.getItem("customerId");
    }
    
    console.log("Final customerId check:", customerId);
    
    if (!customerId) {
      // User not logged in - store room info and redirect to login
      sessionStorage.setItem('selectedRoomId', room.id);
      sessionStorage.setItem('redirectAfterLogin', `/booking/${room.id}`);
      ...
      // User is logged in - go directly to booking page
      console.log("Customer is logged in, navigating to booking page");
      navigate(`/booking/${room.id}`);
    }
  };
  ...
  const checkRoomAvailability = (room) => {
    ...
    // Check multiple availability properties
    const availableRooms = room._availableRooms || room.availableRooms || 0;
    const roomQuantity = room._roomQuantity || room.roomQuantity || 0;
    const isAvailableFlag = room.isAvailable !== false;
    
    return availableRooms > 0 && roomQuantity > 0 && isAvailableFlag;
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="text-sm text-gray-500">
            {rooms.length} total rooms • {filteredRooms.filter(r => checkRoomAvailability(r)).length} available
          </div>
        </div>
        ...
        {filteredRooms.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredRooms.map((room) => {
              ...
              // FIXED: Use enhanced availability check
                ...
                <div key={roomId} className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300">
                  ...
                  <div className="relative h-48 overflow-hidden">
                    ...
                    <div className={`absolute bottom-4 left-4 px-3 py-1 rounded-full text-xs font-semibold ${
                      ...
                      {isAvailable ? `${availableRooms} Available` : 'Fully Booked'}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <p className={`text-lg font-semibold ${
                        checkRoomAvailability(selectedRoom) ? 'text-green-600' : 'text-red-600'
                      }`}>
                        {checkRoomAvailability(selectedRoom) 
                          ? `✅ ${selectedRoom._availableRooms || 0} rooms available` 
                          : '❌ Sold Out'}
                      </p>
                    </div>
                    <div>
