...
export default function CustomerBooking() {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      // Check if room number is included in the response
      const enhancedBookings = data.map(booking => ({
        ...booking,
        // If roomNumber is not in the response, we'll display placeholder
        roomNumber: booking.roomNumber || booking.assignedRoomNumber || null
      }));
